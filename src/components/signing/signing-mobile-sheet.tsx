...
export function SigningMobileSheet({
  ...
  return (
    <>
      <button
        type="button"
        className="fixed bottom-[88px] right-4 z-40 flex cursor-pointer items-center gap-2 rounded-full bg-secondary px-4 py-2.5 font-label-sm text-label-sm text-on-secondary shadow-lg md:hidden"
        onClick={() => onOpenChange(true)}
        aria-label="Open fields and signatures"
      >
      ...
      <div
        className={cn(
          'fixed bottom-0 left-0 z-[70] max-h-[85vh] w-full overflow-y-auto rounded-t-3xl border-t border-outline-variant bg-surface-container-lowest p-6 shadow-2xl transition-transform duration-300 md:hidden',
      ...
      >
        ...
        <div className="mb-4 flex items-center justify-between">
          <h3 className="font-headline-lg text-headline-lg text-primary">Fields and signatures</h3>
          <button
            type="button"
            className="cursor-pointer rounded-full p-2 text-on-surface-variant hover:bg-surface-container"
            aria-label="Close"
            onClick={() => onOpenChange(false)}
          >
        ...
        {fieldItems.length > 0 ? (
          <div className="mb-6 space-y-2">
            <p className="mb-2 font-label-sm text-label-sm uppercase text-on-surface-variant">Required fields</p>
            {fieldItems.map((item) => {
                ...
                <button
                  ...
                  className={cn(
                    ...
                    item.completed
                      ...
                      : isActive
                        ? 'border-status-your-turn bg-accent-light/40'
        ...
        <div className="grid max-h-[30vh] grid-cols-2 gap-3 overflow-y-auto pb-2">
          ...
          {onUploadClick ? (
            <button
              ...
              className="col-span-2 flex h-12 cursor-pointer items-center justify-center gap-2 rounded-xl border border-outline-variant text-on-surface disabled:opacity-50"
            >
